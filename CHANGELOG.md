...
* `koyeb_service` and `koyeb_database` updates additionally wait for the replacement deployment the update rolled out — during a rolling update the service-level status stays healthy on the old deployment, so only the replacement's transition proves the new version is live (mirrors the Python SDK's `koyeb-python-sdk@10210e3`).
